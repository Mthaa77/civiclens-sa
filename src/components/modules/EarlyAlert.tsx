...
const ECRS_TRENDS: Record<string, { quarter: string; score: number }[]> = {
  ...
  CPT: [{ quarter: 'Q1', score: 10 }, { quarter: 'Q2', score: 12 }, { quarter: 'Q3', score: 11 }, { quarter: 'Q4', score: 13 }, { quarter: 'Q5', score: 12 }, { quarter: 'Q6', score: 12 }],
  TSH: [{ quarter: 'Q1', score: 35 }, { quarter: 'Q2', score: 38 }, { quarter: 'Q3', score: 42 }, { quarter: 'Q4', score: 44 }, { quarter: 'Q5', score: 45 }, { quarter: 'Q6', score: 45 }],
  NMB: [{ quarter: 'Q1', score: 40 }, { quarter: 'Q2', score: 42 }, { quarter: 'Q3', score: 45 }, { quarter: 'Q4', score: 48 }, { quarter: 'Q5', score: 50 }, { quarter: 'Q6', score: 50 }],
  SOL: [{ quarter: 'Q1', score: 28 }, { quarter: 'Q2', score: 30 }, { quarter: 'Q3', score: 32 }, { quarter: 'Q4', score: 33 }, { quarter: 'Q5', score: 35 }, { quarter: 'Q6', score: 35 }],
  STE: [{ quarter: 'Q1', score: 8 }, { quarter: 'Q2', score: 9 }, { quarter: 'Q3', score: 10 }, { quarter: 'Q4', score: 10 }, { quarter: 'Q5', score: 11 }, { quarter: 'Q6', score: 11 }],
  RUST: [{ quarter: 'Q1', score: 30 }, { quarter: 'Q2', score: 33 }, { quarter: 'Q3', score: 36 }, { quarter: 'Q4', score: 38 }, { quarter: 'Q5', score: 40 }, { quarter: 'Q6', score: 40 }],
};
...
export default function EarlyAlert() {
  ...
  return (
    <div className="space-y-5 overflow-x-hidden">
      ...
      <motion.div variants={itemSlideUp} initial="hidden" animate="show">
        <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-zinc-400">Section 139 intervention risk prediction</p>
          </div>
      ...
      <motion.div variants={itemFadeIn} initial="hidden" animate="show">
        <div className="flex items-center gap-2 flex-wrap">
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-9 min-h-[44px] sm:min-h-0 sm:h-8 gap-1.5 text-[10px] sm:text-[11px] bg-white/[0.04] backdrop-blur-sm border-white/[0.08] text-zinc-300 hover:bg-[#F43F5E]/10 hover:border-[#F43F5E]/30 hover:text-[#F43F5E] transition-all"
          >
            <FileText className="size-3.5" />
            <span className="hidden sm:inline">Generate MEC Briefing</span><span className="sm:hidden">MEC Brief</span>
          </Button>
          <Button
            variant="outline"
            size="sm"
      ...
      <motion.div variants={itemFadeIn} initial="hidden" animate="show">
        <Card className="glass-card-v2 card-hover-lift overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Risk Dashboard — All Municipalities at a Glance</CardTitle>
            <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Traffic light grid: Early Composite Risk Score (ECRS) per municipality</p>
          </CardHeader>
          <CardContent className="pt-0">
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={cn(
                        'size-10 sm:size-14 md:size-16 rounded-xl cursor-pointer transition-all duration-200 flex flex-col items-center justify-center relative overflow-hidden min-h-[44px]',
                    ...
                    >
                      ...
                      <span className="text-[8px] sm:text-[10px] font-bold text-white/90 relative z-10">{muni.code}</span>
                      <span className="text-[7px] sm:text-[8px] font-medium text-white/70 relative z-10">{score}</span>
                      {/* Mini sparkline for ECRS trend */}
                      {(() => {
                        const trendData = ECRS_TRENDS[muni.code];
                        if (!trendData || trendData.length < 2) return null;
                        const vals = trendData.map(d => d.score);
                        const vMin = Math.min(...vals);
                        const vMax = Math.max(...vals);
                        const vRange = vMax - vMin || 1;
                        const w = 28;
                        const h = 8;
                        const pts = vals.map((v, j) => `${(j / (vals.length - 1)) * w},${h - ((v - vMin) / vRange) * (h - 2) - 1}`).join(' ');
                        return (
                          <svg width={w} height={h} className="relative z-10 mt-0.5 hidden sm:block" viewBox={`0 0 ${w} ${h}`}>
                            <polyline points={pts} fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth={1} strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                        );
                      })()}
                    </button>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-3">
              <span className="text-[9px] sm:text-[10px] text-zinc-400">Risk Level:</span>
              {[
                ...
                <div key={l.label} className="flex items-center gap-1">
                  ...
                  <span className="text-[9px] sm:text-[10px] text-zinc-400">{l.label}</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="glass-card-v2 card-hover-lift overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">30-Day Risk Forecast</CardTitle>
                <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Municipalities with highest predicted ECRS increase — ML model prediction</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {RISK_FORECASTS
                ...
                .sort((a, b) => (b.predictedECRS - b.currentECRS) - (a.predictedECRS - a.currentECRS))
                .slice(0, 3)
                .map((forecast, i) => {
                  const increase = forecast.predictedECRS - forecast.currentECRS;
                  const trendColor = getRiskColor(forecast.predictedECRS);
                  return (
                    <motion.div
                      key={forecast.code}
                      ...
                      className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3 sm:p-4 hover:bg-white/[0.04] hover:border-white/[0.1] transition-all relative overflow-hidden group"
                      style={{ borderLeftWidth: '3px', borderLeftColor: trendColor }}
                    >
                      <div className="absolute inset-0 opacity-[0.04]" style={{ background: `linear-gradient(135deg, ${trendColor}, transparent 60%)` }} />
                      <div className="relative flex items-center gap-2 mb-2">
                        <Building2 className="size-3.5" style={{ color: trendColor }} />
                        <span className="text-xs sm:text-sm font-semibold text-zinc-200">{forecast.municipality}</span>
                        <Badge
                          variant="outline"
                          className="text-[8px] h-4 px-1.5 ml-auto shrink-0 bg-red-500/15 text-red-400 border-red-500/25"
                        >
                          +{increase} ECRS
                        </Badge>
                      </div>
                      <div className="relative flex items-center gap-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm font-bold tabular-nums" style={{ color: getRiskColor(forecast.currentECRS) }}>{forecast.currentECRS}</span>
                        </div>
                        <ArrowRight className="size-3 text-zinc-500" />
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] text-zinc-400">Predicted:</span>
                          <span className="text-sm font-bold tabular-nums" style={{ color: trendColor }}>{forecast.predictedECRS}</span>
                        </div>
                        <TrendingUp className="size-3.5 text-red-400 ml-auto shrink-0" />
                      </div>
                      {/* Mini progress bar showing ECRS position */}
                      <div className="relative mt-2 h-1.5 rounded-full bg-white/[0.04] overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${forecast.currentECRS}%` }}
                          transition={{ delay: 0.3 + i * 0.1, duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
                          className="h-full rounded-full"
                          style={{ background: `linear-gradient(90deg, ${getRiskColor(forecast.currentECRS)}, ${trendColor})` }}
                        />
                        {/* Predicted marker */}
                        <motion.div
                          initial={{ left: 0, opacity: 0 }}
                          animate={{ left: `${forecast.predictedECRS}%`, opacity: 1 }}
                          transition={{ delay: 0.5 + i * 0.1, duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
                          className="absolute top-0 h-full w-0.5 rounded-full bg-white/80"
        ...
        <Card className="glass-card-v2 card-hover-lift overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200 flex items-center gap-2">
                  <Eye className="size-3 sm:size-4" style={{ color: ACCENT_FROM }} />
                  Risk Signal Feed
                </CardTitle>
                <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Latest detected risk signals — hover for details</p>
              </div>
                      ...
                      >
                        <UITooltip>
                          <TooltipTrigger asChild>
                            <div
                              className="flex items-center gap-2 sm:gap-3 rounded-lg border border-white/[0.06] bg-white/[0.02] p-2.5 sm:p-3 hover:border-white/[0.12] hover:bg-white/[0.04] transition-all cursor-default relative overflow-hidden"
                              style={{
                                borderLeftWidth: '3px',
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1 sm:gap-1.5 min-w-0 shrink-0 relative z-10">
                                <Building2 className="size-2.5 sm:size-3 text-zinc-500" />
                                <span className="text-[10px] sm:text-[11px] font-semibold text-zinc-200 truncate">{signal.municipality}</span>
                              </div>
                              {/* Signal type */}
                              <span className="text-[9px] sm:text-[11px] text-zinc-400 flex-1 truncate relative z-10">{signal.type}</span>
                              {/* Urgency badge */}
                              {urgency.label && (
                                <Badge
                                  variant="outline"
                              ...
                              <Button
                                ...
                                className="h-7 sm:h-6 gap-1 text-[8px] sm:text-[9px] text-zinc-400 hover:text-white shrink-0 relative z-10 min-h-[44px] sm:min-h-0 hover:bg-gradient-to-r hover:from-[#F43F5E]/20 hover:to-[#DC2626]/20 hover:border hover:border-[#F43F5E]/30 transition-all"
                              >
                                Investigate
      ...
      <motion.div variants={containerStagger} initial="hidden" animate="show" className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {/* ECRS Trend - Enhanced with Area Fill, Reference Zones, Pulsing Dots */}
        <motion.div variants={itemFadeIn} className="xl:col-span-2">
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: `linear-gradient(90deg, ${ACCENT_FROM}, ${ACCENT_TO})` }} />
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">
                    ECRS Trend — {selectedMuniData?.name ?? selectedMuni}
                  </CardTitle>
                  <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">6-quarter Early Composite Risk Score trajectory</p>
                </div>
                <div className="flex items-center gap-2">
        ...
        <motion.div variants={itemFadeIn}>
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: `linear-gradient(90deg, ${ACCENT_FROM}, ${ACCENT_TO})` }} />
            <CardHeader className="pb-2">
              <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Automated Briefing Generator</CardTitle>
              <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Generate MEC briefing document</p>
            </CardHeader>
            <CardContent className="pt-0 space-y-3">
              <div className="rounded-lg border border-white/[0.06] p-3" style={{ borderLeftWidth: '3px', borderLeftColor: ACCENT_FROM }}>
                <p className="text-[10px] sm:text-[11px] font-medium text-zinc-400">Selected Municipality</p>
                <p className="text-xs sm:text-sm font-semibold text-zinc-200 mt-1">{selectedMuniData?.name ?? selectedMuni}</p>
                <div className="grid grid-cols-2 gap-2 mt-2 text-[9px] sm:text-[10px]">
      ...
      <motion.div variants={itemSlideUp} initial="hidden" animate="show">
        <Card className="glass-card-v2 card-hover-lift overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center gap-2">
              <Brain className="size-4" style={{ color: ACCENT_FROM }} />
              <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Q4 2026 Risk Forecast</CardTitle>
              <Badge className="text-[9px] sm:text-[9px] h-5 px-1.5" style={{ background: `${ACCENT_FROM}15`, color: ACCENT_FROM, borderColor: `${ACCENT_FROM}25` }}>
                ML Predicted
              </Badge>
            </div>
            <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Predicted ECRS direction for next quarter — ▲ Up = increasing risk, ▼ Down = decreasing risk, → Stable</p>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
              {RISK_FORECASTS.map((fc, i) => (
                <motion.div
                  ...
                  className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-2 sm:p-3 hover:border-white/[0.12] transition-all"
                  style={{
                    borderLeftWidth: '3px',
                ...
                >
                  <div className="flex items-center gap-1 sm:gap-1.5 mb-2">
                    ...
                    <span className="text-[9px] sm:text-[10px] font-semibold text-zinc-200 truncate">{fc.municipality}</span>
                  </div>
                  <div className="flex items-center justify-between mb-1.5">
                    <div>
                      <p className="text-[9px] text-zinc-500">Current</p>
                      <p className="text-xs sm:text-sm font-bold tabular-nums" style={{ color: getRiskColor(fc.currentECRS) }}>
                        {fc.currentECRS}
                      </p>
                    </div>
                    <div className="flex items-center">
                      {fc.direction === 'up' ? (
                        <TrendingUp className="size-4 sm:size-5 text-red-400" />
                      ) : fc.direction === 'down' ? (
                        <TrendingDown className="size-4 sm:size-5 text-emerald-400" />
                      ) : (
                        <Minus className="size-4 sm:size-5 text-amber-400" />
                      )}
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] text-zinc-500">Predicted</p>
                      <p className="text-xs sm:text-sm font-bold tabular-nums" style={{ color: getRiskColor(fc.predictedECRS) }}>
                        {fc.predictedECRS}
                      </p>
                    </div>
                  </div>
      ...
      <motion.div variants={containerStagger} initial="hidden" animate="show" className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {/* Risk Distribution - Enhanced with outer ring glow + hover scale on legend */}
        <motion.div variants={itemFadeIn}>
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: `linear-gradient(90deg, ${ACCENT_FROM}, ${ACCENT_TO})` }} />
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Risk Distribution</CardTitle>
              </div>
              <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Severity breakdown across monitored municipalities</p>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="flex items-center gap-6 flex-col sm:flex-row">
                <div className="relative w-[130px] h-[130px] sm:w-[160px] sm:h-[160px] shrink-0">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={RISK_DISTRIBUTION} cx="50%" cy="50%" innerRadius={35} outerRadius={55} paddingAngle={3} dataKey="value" strokeWidth={0}>
                ...
                <div className="flex-1 space-y-2 sm:space-y-3 w-full">
                  {RISK_DISTRIBUTION.map((item) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] sm:text-xs font-bold text-zinc-200 tabular-nums">{item.value}</span>
                      <span className="text-[9px] sm:text-[10px] text-zinc-400 tabular-nums w-8 text-right">{((item.value / 12) * 100).toFixed(0)}%</span>
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Key Risk Indicators</CardTitle>
              </div>
              <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Top risk factors driving Section 139 triggers</p>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="relative overflow-hidden rounded-lg border border-white/[0.06] p-2.5 sm:p-3 hover:border-white/[0.12] transition-all" style={{ borderLeftWidth: '3px', borderLeftColor: item.color }}>
                    <div className="absolute inset-0 opacity-[0.03]" style={{ background: `linear-gradient(135deg, ${item.color}, transparent)` }} />
                    <div className="relative flex items-center justify-between mb-1.5">
                      <span className="text-[10px] sm:text-xs font-medium text-zinc-200">{item.label}</span>
                      <span className="text-[10px] sm:text-xs font-bold tabular-nums" style={{ color: item.color }}>{item.pct}%</span>
                    </div>
          ...
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Municipal Risk Comparison Radar</CardTitle>
              </div>
              <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Compare up to 3 municipalities across 6 risk dimensions</p>
            </CardHeader>
            <CardContent className="pt-0 space-y-3">
                ...
                {MOCK_MUNICIPALITIES.map((muni) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        'px-2 py-1 rounded-md text-[9px] sm:text-[10px] font-medium transition-all border min-h-[36px] sm:min-h-0',
                        isSelected
                          ? 'text-white'
          ...
          <Card className="glass-card-v2 card-hover-lift overflow-hidden">
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Intervention Cost Estimator</CardTitle>
              </div>
              <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Estimate Section 139 intervention costs and ROI</p>
            </CardHeader>
            <CardContent className="pt-0 space-y-3">
              {/* Municipality selector */}
              <div>
                <label className="text-[9px] sm:text-[10px] font-medium text-zinc-400 uppercase tracking-wider mb-1 block">Select Municipality</label>
                <Select value={costMuni} onValueChange={setCostMuni}>
                  <SelectTrigger className="h-9 sm:h-8 text-[10px] sm:text-[11px] bg-white/[0.03] border-white/[0.08] text-zinc-200 min-h-[44px] sm:min-h-0">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={item.label} className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-2 sm:p-2.5 hover:border-white/[0.12] transition-all" style={{ borderLeftWidth: '3px', borderLeftColor: item.color }}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        <item.icon className="size-3 sm:size-3.5" style={{ color: item.color }} />
                        <span className="text-[10px] sm:text-[11px] text-zinc-300">{item.label}</span>
                      </div>
                      <span className="text-[10px] sm:text-[11px] font-bold text-zinc-200 tabular-nums">
              ...
              <div className="rounded-xl border border-white/[0.08] bg-gradient-to-r from-white/[0.03] to-white/[0.01] p-3" style={{ boxShadow: `0 0 12px ${ACCENT_FROM}08` }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] sm:text-[10px] font-medium text-zinc-400 uppercase tracking-wider">Total Estimated Annual Cost</span>
                </div>
                <p className="text-base sm:text-lg font-bold tabular-nums" style={{ color: ACCENT_FROM }}>
                  {formatZAR(costEstimate.totalAnnual.min)} — {formatZAR(costEstimate.totalAnnual.max)}
                </p>
              </div>

              {/* ROI Indicator */}
              <div className="rounded-xl border border-white/[0.06] p-2.5 sm:p-3 relative overflow-hidden" style={{ borderColor: costEstimate.roi > 0 ? '#10B98130' : '#EF444430' }}>
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-[10px] sm:text-[11px] font-semibold" style={{ color: costEstimate.roi > 0 ? '#10B981' : '#EF4444' }}>
                      ROI Indicator
                    </span>
                    <Badge
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-[8px] sm:text-[9px] text-zinc-500">Projected Annual Savings</p>
                      <p className="text-xs sm:text-sm font-bold tabular-nums text-emerald-400">{formatZAR(costEstimate.projectedSavings)}</p>
                    </div>
                    <div>
                      <p className="text-[8px] sm:text-[9px] text-zinc-500">ROI vs Max Cost</p>
                      <p className="text-xs sm:text-sm font-bold tabular-nums" style={{ color: costEstimate.roi > 0 ? '#10B981' : '#EF4444' }}>
              ...
              {costMuniData && (
                <div className="text-[8px] sm:text-[9px] text-zinc-500 flex items-center gap-2">
      ...
      <motion.div variants={itemSlideUp} initial="hidden" animate="show">
        <Card className="glass-card-v2 card-hover-lift overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-xs sm:text-sm font-semibold text-zinc-200">Section 139 Intervention History</CardTitle>
              <Badge className="text-[8px] sm:text-[9px] h-5 px-1.5" style={{ background: `${ACCENT_FROM}15`, color: ACCENT_FROM, borderColor: `${ACCENT_FROM}25` }}>
                {INTERVENTIONS.filter((int) => int.status === 'Active').length} Active
              </Badge>
            </div>
            <p className="text-[10px] sm:text-[11px] text-zinc-400 mt-0.5">Timeline of past and current interventions</p>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="relative">
                ...
                return (
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      className={cn(
                        'rounded-lg border border-white/[0.06] p-2.5 sm:p-3 transition-all relative overflow-hidden',
                      ...
                      <div className="relative">
                        <div className="flex items-start justify-between mb-1.5 gap-2">
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-xs font-semibold text-zinc-200">{int.municipality}</p>
                            <p className="text-[10px] sm:text-[11px] text-zinc-300 mt-0.5">{int.reason}</p>
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 text-[9px] sm:text-[10px]">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-5 mt-2 gap-1 text-[9px] sm:text-[9px] text-zinc-500 hover:text-[#F43F5E] p-0 min-h-[44px] sm:min-h-0"
